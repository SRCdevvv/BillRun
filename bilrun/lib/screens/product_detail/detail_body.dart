...
class DetailScreenBody extends StatelessWidget {
  ...
  const DetailScreenBody(this.product);
  ...
  Widget build(BuildContext context) {
    ...
    int differenceMinute = int.parse(
        DateTime.now().difference(product.createdAt).inMinutes.toString());
        ...
        children: [
          ...
          Padding(
            ...
            child: // 뚜디니는졸귀야 님의 다른 물품
                Text("${product.user.nickname} 님의 다른 물품",
                    style: const TextStyle(
                        color: const Color(0xff000000),
                        fontWeight: FontWeight.w700,
                        fontFamily: "NotoSansCJKkr",
          ...
          FutureBuilder(
              future: EtcProductController.fetchEtcProducts(product.user.id),
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                ...
                if (snapshot.hasError) {
                  return Text("etc error");
                } else {
                  return Padding(
                    padding: const EdgeInsets.fromLTRB(24, 10, 24, 0),
                    child: Obx(() {
                      if (EtcProductController.isLoading.value)
                        return Center(child: CircularProgressIndicator());
                      else
